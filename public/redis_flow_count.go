	...
	go func() {
		...
		for {
			...
			if err := RedisConfPipline(func(c redis.Conn) {
				...
				fmt.Println("redisConf err:",err)
				continue
			}

			totalDayCount, err := reqCounter.GetDayData(currentTime)
			if err != nil {
				fmt.Println("totalDayCount err:",err)
				continue
			}
			...
			tickerCount = totalDayCount - reqCounter.TotalCount
			if nowUnix > reqCounter.Unix {
				reqCounter.TotalCount = totalDayCount
