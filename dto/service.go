...
type ServiceDeleteInput struct {
	ID int64 `json:"id" form:"id" comment:"服务ID" example:"1" validate:"required"`//关键词
}
...
type ServiceAddHttpInput struct {
	ServiceName string `json:"service_name" form:"service_name" comment:"服务名" example:"test_http_service_indb" validate:"required,is_valid_service_name"` //服务名
	...
	Rule           string `json:"rule" form:"rule" comment:"接入路径：域名或者前缀" example:"/test_http_service_indb" validate:"required,is_valid_rule"` //域名或者前缀
	...
	HeaderTransfor string `json:"header_transfor" form:"header_transfor" comment:"header转换" example:"" validate:"valid_header_transform"`   //header转换
	...
	ClientipFlowLimit int    `json:"clientip_flow_limit" form:"clientip_flow_limit" comment:"bs 客户端ip限流	" example:"" validate:"min=0"` //客户端ip限流
	...
	IpList                 string `json:"ip_list" form:"ip_list" comment:"ip列表" example:"127.0.0.1:80" validate:"required,valid_ip_port_list"`                //ip列表
	WeightList             string `json:"weight_list" form:"weight_list" comment:"bs权重列表" example:"50" validate:"required,valid_weight_list"`             //权重列表
...
type ServiceListInput struct {
	...
	PageSize int `json:"page_size" form:"page_size" comment:"每页条数" example:"20" validate:"required"`//页面
}
...
type ServiceListOutput struct {
	...
	List []ServiceListItemOutput `json:"list" form:"list" comment:"列表" example:"1" validate:""`//列表
}

type ServiceStatisticsOutput struct {
	Today []int64 `json:"today" form:"yesterday" comment:"今日流量" example:"" validate:""`//总数
	Yesterday []int64 `json:"yesterday" form:"yesterday" comment:"昨日流量" example:"" validate:""`//总数
}
//tcp
type ServiceAddTcpInput struct {
	ServiceName       string `json:"service_name" form:"service_name" comment:"服务名称" validate:"required,is_valid_service_name"`
...
type ServiceAddGrpcInput struct {
	...
	ServiceDesc       string `json:"service_desc" form:"service_desc" comment:"服务描述" validate:"required"`
	Port              int    `json:"port" form:"port" comment:"端口，需要设置8001-8999范围内" validate:"required,min=8001,max=8999"`
	HeaderTransfor    string `json:"header_transfor" form:"header_transfor" comment:"metadata转换" validate:"valid_header_transform"`
	OpenAuth          int    `json:"open_auth" form:"open_auth" comment:"是否开启权限验证" validate:""`
	BlackList         string `json:"black_list" form:"black_list" comment:"黑名单IP，以逗号间隔，白名单优先级高于黑名单" validate:"valid_ip_list"`
	WhiteList         string `json:"white_list" form:"white_list" comment:"白名单IP，以逗号间隔，白名单优先级高于黑名单" validate:"valid_ip_list"`
	WhiteHostName     string `json:"white_host_name" form:"white_host_name" comment:"白名单主机，以逗号间隔" validate:"valid_ip_list"`
